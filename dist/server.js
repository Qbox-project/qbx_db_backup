...
async function pruneS3Bucket(client, policy) {
  ...
  const result = { deletedKeys: [], errors: [] };
...
var LocalFileSink = class {
  ...
  filePath;
  async open(options) {
    await (0, import_promises4.mkdir)(import_node_path3.default.dirname(this.filePath), { recursive: true });
...
var UploadSink = class {
  ...
  async open(options) {
    ...
    const dir = await (0, import_promises4.mkdtemp)(import_node_path3.default.join(root, "qbx-db-backup-"));
...
var S3Sink = class {
  ...
  async open(options) {
    ...
    const dir = await (0, import_promises5.mkdtemp)(import_node_path4.default.join(root, "qbx-s3-sink-"));
...
async function readLastRunAt(dir) {
  try {
    const raw = await (0, import_promises6.readFile)(import_node_path5.default.join(dir, STATE_FILE_NAME), "utf8");
...
async function writeLastRunAt(dir, lastRunAt) {
  ...
  await (0, import_promises6.writeFile)(import_node_path5.default.join(dir, STATE_FILE_NAME), `${JSON.stringify({ lastRunAt })}
`, "utf8");
}
...
async function requestJob(trigger) {
  ...
  const created = await api.createJob(trigger).catch((failure) => {
    throw new DestinationError(failure);
  });
...
async function runS3WithLocalFallback() {
  try {
    await runS3Backup();
  } catch (failure) {
